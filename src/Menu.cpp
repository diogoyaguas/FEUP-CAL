...
void MenuBase::printOptions(const string& title, const vector<string>& options) {
	...
	options_nonconst.push_back("Exit");
	...
	for (size_t i = 0; i < options_nonconst.size(); i++) {
		if (options_nonconst[i].size() > maxOptionSize) {
			maxOptionSize = options_nonconst[i].size();
		}
	}
	maxOptionSize += 10;
	...
	for (size_t i = 0; i < options_nonconst.size(); i++) {
		options_nonconst[i].resize(maxOptionSize, ' ');
	...
	//Opcao Sair
	cout << separateFromBorder << x1 << separateMenu << "E - " << options_nonconst.back() << x1 << endl << separateFromBorder << spaceLine << endl;
